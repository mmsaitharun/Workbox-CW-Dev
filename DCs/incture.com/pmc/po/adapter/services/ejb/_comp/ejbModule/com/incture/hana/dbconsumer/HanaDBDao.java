package com.incture.hana.dbconsumer;
...
public class HanaDBDao {
	...
	public ProcessEventsDto readProcessEvent(String processId) throws NoResultFault, ExecutionFault {
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][readProcessEvent] started with processId : " +processId);
		...
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][readProcessEvent] ended with processEventsDto : " +processEventsDto);
	...
	public ResponseDto updateProcessEvents(ProcessEventsDto processEventsDto) {
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][updateProcessEvents] started with processEventsDto : " +processEventsDto);
		ResponseDto responseDto = new ResponseDto();
		int result = 0;
		...
		try {
			...
			System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][updateProcessEvents] merge completed eventsDto : "+eventsDto);
			
			String updateQuery = "UPDATE PROCESS_EVENTS SET PROCESS_ID = '"+eventsDto.getProcessId()+"', "
					+ "COMPLETED_AT = "+(ServicesUtil.isEmpty((eventsDto.getCompletedAt())) ? null : "'"+(dateFormatter.format(eventsDto.getCompletedAt())+"'"))+", NAME = '"+eventsDto.getName()+"', REQUEST_ID = '"+eventsDto.getRequestId()+"', "
							+ "STARTED_AT = "+(ServicesUtil.isEmpty((eventsDto.getStartedAt())) ? null : "'"+(dateFormatter.format(eventsDto.getStartedAt())+"'"))+", STARTED_BY = '"+eventsDto.getStartedBy()+"', STARTED_BY_DISP = '"+eventsDto.getStartedByDisplayName()+"', STATUS = '"+eventsDto.getStatus()+"', SUBJECT = '"+eventsDto.getSubject()+"' WHERE PROCESS_ID = '"+eventsDto.getProcessId()+"'";
		...
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][updateProcessEvents] ended with responseDto : " +responseDto);
		return responseDto;
	}
	...
	private ProcessEventsDto mergeProcessEventsDto(ProcessEventsDto processEventsDto, String processId) throws NoResultFault, ExecutionFault {
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][mergeProcessEventsDto] started with processEventsDto : " +processEventsDto);
		...
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][mergeProcessEventsDto] ended with eventsDto : " +eventsDto);
		return eventsDto;
	}
...
//					taskOwnersDto.setTaskOwnerEmail(UmeUtil.getEmailIdByUserId(taskEventsDto.getCurrentProcessor()));
				...
				System.err.println("[PMC][DatabaseManagementFacade][saveAndUpdateTask][NoResultFault]thrown and trying new insert");
				try {
	...
	private void updateRequestId(String processId, String requestId) {
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][updateRequestId] started with request id : " +requestId);
		ProcessEventsDto processEventsDto = new ProcessEventsDto();
//		processEventsDto.setProcessId(processId);
		...
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][updateRequestId] ended updating");
	}
...
//				throw new NoResultFault("noRecordFound");
		...
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][updateProcessedOwner] ended" );
	}
	...
	public ResponseDto insertIntoTaskEvents(TaskEventsDto taskEventsDto) {
		...
		System.err.println("createdDate : "+taskEventsDto.getCreatedAt());
		System.err.println("formatted createdDate : "+dateFormatter.format(taskEventsDto.getCreatedAt()));
		String eQuery = "INSERT INTO TASK_EVENTS(COMPLETED_AT, COMP_DEADLINE, CREATED_AT, CUR_PROC, CUR_PROC_DISP, DESCRIPTION, FORWARDED_AT, FORWARDED_BY, NAME, ORIGIN, PRIORITY, PROC_NAME, STATUS, STATUS_FLAG, SUBJECT, TASK_MODE, TASK_TYPE, URL, EVENT_ID, PROCESS_ID) VALUES "
				+ "('"+(ServicesUtil.isEmpty(taskEventsDto.getCompletedAt()) ? null : dateFormatter.format(taskEventsDto.getCompletedAt()))+"', "
						+ "'"+(ServicesUtil.isEmpty(taskEventsDto.getCompletionDeadLine()) ? null : dateFormatter.format(taskEventsDto.getCompletionDeadLine()))+"', "
								+ "'"+(ServicesUtil.isEmpty(dateFormatter.format(taskEventsDto.getCreatedAt())) ? null : (dateFormatter.format(taskEventsDto.getCreatedAt())))+"', "
										+ "'"+taskEventsDto.getCurrentProcessor()+"', '"+taskEventsDto.getCurrentProcessorDisplayName()+"', '"+taskEventsDto.getDescription()+"', "
												+ "'"+(ServicesUtil.isEmpty((taskEventsDto.getForwardedAt())) ? null : (dateFormatter.format(taskEventsDto.getForwardedAt())))+"', '"+taskEventsDto.getForwardedBy()+"', '"+taskEventsDto.getName()+"', '"+taskEventsDto.getOrigin()+"', '"+taskEventsDto.getPriority()+"', '"+taskEventsDto.getProcessName()+"', '"+taskEventsDto.getStatus()+"', '"+taskEventsDto.getStatusFlag()+"', '"+taskEventsDto.getSubject()+"', '"+taskEventsDto.getTaskMode()+"', '"+taskEventsDto.getTaskType()+"', '"+taskEventsDto.getUrl()+"', '"+taskEventsDto.getEventId()+"', '"+taskEventsDto.getProcessId()+"')";
		
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][insertIntoTaskEvents][query] : "+eQuery);
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][insertIntoTaskEvents][after eQuery]");
...
//			String insertQuery = "INSERT INTO TASK_EVENTS(COMPLETED_AT, COMP_DEADLINE, CREATED_AT, CUR_PROC, CUR_PROC_DISP, DESCRIPTION, FORWARDED_AT, FORWARDED_BY, NAME, ORIGIN, PRIORITY, PROC_NAME, STATUS, STATUS_FLAG, SUBJECT, TASK_MODE, TASK_TYPE, URL, EVENT_ID, PROCESS_ID) VALUES ('"+dateFormatter.format(taskEventsDto.getCompletedAt())+"', '"+taskEventsDto.getCompletionDeadLine()+"', '"+dateFormatter.format(taskEventsDto.getCreatedAt())+"', '"+taskEventsDto.getCurrentProcessor()+"', '"+taskEventsDto.getCurrentProcessorDisplayName()+"', '"+taskEventsDto.getDescription()+"', '"+dateFormatter.format(taskEventsDto.getForwardedAt())+"', '"+taskEventsDto.getForwardedBy()+"', '"+taskEventsDto.getName()+"', '"+taskEventsDto.getOrigin()+"', '"+taskEventsDto.getPriority()+"', '"+taskEventsDto.getProcessName()+"', '"+taskEventsDto.getStatus()+"', '"+taskEventsDto.getStatusFlag()+"', '"+taskEventsDto.getSubject()+"', '"+taskEventsDto.getTaskMode()+"', '"+taskEventsDto.getTaskType()+"', '"+taskEventsDto.getUrl()+"', '"+taskEventsDto.getEventId()+"', '"+taskEventsDto.getProcessId()+"')";
			String insertQuery = "INSERT INTO TASK_EVENTS(COMPLETED_AT, COMP_DEADLINE, CREATED_AT, CUR_PROC, CUR_PROC_DISP, DESCRIPTION, FORWARDED_AT, FORWARDED_BY, NAME, ORIGIN, PRIORITY, PROC_NAME, STATUS, STATUS_FLAG, SUBJECT, TASK_MODE, TASK_TYPE, URL, EVENT_ID, PROCESS_ID) VALUES "
					+ "("+(ServicesUtil.isEmpty(taskEventsDto.getCompletedAt()) ? null : "'"+dateFormatter.format(taskEventsDto.getCompletedAt())+"'")+", "
					+ ""+(ServicesUtil.isEmpty(taskEventsDto.getCompletionDeadLine()) ? null : "'"+dateFormatter.format(taskEventsDto.getCompletionDeadLine())+"'")+", "
							+ ""+(ServicesUtil.isEmpty(dateFormatter.format(taskEventsDto.getCreatedAt())) ? null : "'"+(dateFormatter.format(taskEventsDto.getCreatedAt()))+"'")+", "
									+ "'"+taskEventsDto.getCurrentProcessor()+"', '"+taskEventsDto.getCurrentProcessorDisplayName()+"', '"+taskEventsDto.getDescription()+"', "
	...
	public ResponseDto updateTaskEvents(TaskEventsDto taskEventsDto) throws NoResultFault {
		ResponseDto responseDto = new ResponseDto();
		int result = 0;
		...
		try{
			...
			String updateQuery = "UPDATE TASK_EVENTS SET COMPLETED_AT = "+(ServicesUtil.isEmpty((eventsDto.getCompletedAt())) ? null : "'"+(dateFormatter.format(eventsDto.getCompletedAt()))+"'")+","
					+ " COMP_DEADLINE = "+(ServicesUtil.isEmpty((eventsDto.getCompletionDeadLine())) ? null : "'"+(dateFormatter.format(eventsDto.getCompletionDeadLine()))+"'")+", "
							+ "CREATED_AT = "+(ServicesUtil.isEmpty((eventsDto.getCreatedAt())) ? null : "'"+(dateFormatter.format(eventsDto.getCreatedAt()))+"'")+", CUR_PROC = '"+eventsDto.getCurrentProcessor()+"', CUR_PROC_DISP = '"+eventsDto.getCurrentProcessorDisplayName()+"', DESCRIPTION = '"+eventsDto.getDescription()+"', "
									+ "FORWARDED_AT = "+(ServicesUtil.isEmpty((eventsDto.getForwardedAt())) ? null : "'"+(dateFormatter.format(eventsDto.getForwardedAt()))+"'")+", FORWARDED_BY = '"+eventsDto.getForwardedBy()+"', NAME = '"+eventsDto.getName()+"', ORIGIN = '"+eventsDto.getOrigin()+"', PRIORITY = '"+eventsDto.getPriority()+"', PROC_NAME = '"+eventsDto.getProcessName()+"', STATUS = '"+eventsDto.getStatus()+"', STATUS_FLAG = '"+eventsDto.getStatusFlag()+"', SUBJECT = '"+eventsDto.getSubject()+"', TASK_MODE = '"+eventsDto.getTaskMode()+"', TASK_TYPE = '"+eventsDto.getTaskType()+"', URL = '"+eventsDto.getUrl()+"', EVENT_ID = '"+eventsDto.getEventId()+"', PROCESS_ID = '"+eventsDto.getProcessId()+"'";
...
//			insertIntoTaskEvents(taskEventsDto);
	...
	public TaskEventsDto readTaskEvent(String taskId) throws ExecutionFault, NoResultFault {
		...
		if(!ServicesUtil.isEmpty(taskId)){
			...
			if(ServicesUtil.isEmpty(taskEventsDto)){
//				this.insertIntoTaskEvents(taskEventsDto);
				throw new NoResultFault("noRecordFound" +  "#" +taskId);
			}
		}
	...
	public ResponseDto updateTaskOwners(TaskOwnersDto taskOwnersDto) throws NoResultFault {
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][updateTaskOwners] started with : "+taskOwnersDto);
		ResponseDto responseDto = new ResponseDto();
		...
		try{
			...
			String updateQuery = "UPDATE TASK_OWNERS SET IS_PROCESSED = '"+((ownersDto.getIsProcessed())? '1' : '0' )+"', TASK_OWNER_EMAIL = '"+ownersDto.getTaskOwnerEmail()+"', TASK_OWNER_DISP = '"+ownersDto.getTaskOwnerDisplayName()+"', EVENT_ID = '"+ownersDto.getEventId()+"', TASK_OWNER = '"+ownersDto.getTaskOwner()+"' WHERE EVENT_ID = '"+ownersDto.getEventId()+"' AND TASK_OWNER = '"+ownersDto.getTaskOwner()+"'";
...
//			insertIntoTaskOwners(taskOwnersDto);
		...
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][updateTaskOwners] ended with : "+responseDto);
		return responseDto;
	}
	...
	public TaskOwnersDto readTaskOwners(String taskId, String taskOwner) throws ExecutionFault, NoResultFault {
		...
		if(!ServicesUtil.isEmpty(taskId) && !ServicesUtil.isEmpty(taskOwner)){
			String selectTaskOwner = "SELECT * FROM TASK_OWNERS WHERE EVENT_ID = '"+taskId.trim()+"' AND TASK_OWNER = '"+taskOwner.trim()+"'";
			try {
	...
	private TaskOwnersDto getTaskOwnersDtoFromResultSet(ResultSet resultSet) {
		...
		System.err.println("[PMC][POADAPTER][HANA][HanaDBDao][getTaskOwnersDtoFromResultSet] ended with ownersDto : "+ownersDto);
		return ownersDto;
	}
	...
	public void updateAutomatedTask(String processId, Date CompletedDate) {
		...
		if (!ServicesUtil.isEmpty(processId)) {
			String updateQuery = "Update TASK_EVENTS p SET p.STATUS = \'COMPLETED\', p.COMPLETED_AT = '"+(ServicesUtil.isEmpty((CompletedDate)) ? null : dateFormatter.format(dateFormatter.format(CompletedDate)))+"' where p.PROCESS_ID = '"+processId+"' and p.TASK_TYPE = \'Automated\' and p.STATUS = \'STARTED\'";
			int i = con.update(updateQuery);
