...
import com.incture.pmc.dto.RemainderDto;
import com.incture.pmc.dto.RemainderMailDto;
import com.incture.pmc.dto.ResponseMessage;
import com.incture.pmc.util.MailService;
	...
	private ResponseMessage sendProcRemMail(RemainderDto dto) {
		...
		if(!ServicesUtil.isEmpty(dto.getAction())){
			if(PMCConstant.SEND_NOTIFICATION.equalsIgnoreCase(dto.getAction())){
				...
				String emailOfStartedBy = umeConsumer.getUserEmailById(dto.getUserId());
	...
	private ResponseMessage sendTaskRemMail(RemainderDto dto) {
		...
		if(!ServicesUtil.isEmpty(dto.getAction())){
			if(dto.getStatus().equalsIgnoreCase("READY")){
				...
				if(PMCConstant.SEND_NOTIFICATION.equalsIgnoreCase(dto.getAction())){
					String emailIds= "";
					String ownerEmail = umeConsumer.getUserEmailById(dto.getUserId());
					mailer = new MailService();
					rmDto = new RemainderMailDto();
