...
import com.incture.pmc.entity.TaskEventsDoPK;
...
public class TaskEventsDao extends BaseDao<TaskEventsDo, TaskEventsDto> {
	...
	public List<Object[]> getUserTaskCount(String userId, String processName, String requestId, String labelValue, String status, Date startDate, Date endDate) throws NoResultFault {
		...
		String tempQuery = "";
		String query = "select count(te.CREATED_AT) AS TASK_COUNT, trunc(te.CREATED_AT) AS CREATED_DATE from task_events te left join task_owners tw on te.event_id = tw.event_id where tw.task_owner ='"
				+ userId + "'";
		String groupQuery = " group by trunc(te.CREATED_AT) ORDER BY trunc(te.CREATED_AT)";
		...
		if (!ServicesUtil.isEmpty(startDate) && !ServicesUtil.isEmpty(endDate)) {
			tempQuery = tempQuery + " and te.CREATED_AT between TO_DATE('" + dateFormatter.format(startDate) + "', 'DD/MM/YY hh:mi:ss AM') and TO_DATE('" + dateFormatter.format(endDate) + "', 'DD/MM/YY hh:mi:ss PM')";
		}
	...
	public List<Object[]> getTaskCountByOwner(String userId, String processName, String requestId, String labelValue, String status, Date startDate, Date endDate) {
		...
		String query = "SELECT TRUNC(te." + queryColumn + ") AS TASK_DATE, COUNT(TRUNC(te." + queryColumn
				...
				+ userId + "'";
		if (!ServicesUtil.isEmpty(processName) && !processName.equals(PMCConstant.SEARCH_ALL) && processName.contains("'")) {
		...
		if (!ServicesUtil.isEmpty(startDate) && !ServicesUtil.isEmpty(endDate)) {
			...
			tempQuery = tempQuery + "TO_DATE('"+dateFormatter.format(startDate) + "', 'DD/MM/YY hh:mi:ss AM') and TO_DATE('" + dateFormatter.format(endDate) + "', 'DD/MM/YY hh:mi:ss PM')";
		}
		if (!PMCConstant.SEARCH_COMPLETED.equalsIgnoreCase(status))
			tempQuery = tempQuery + " and pe.status='" + PMCConstant.PROCESS_STATUS_IN_PROGRESS + "'";
		String groupQuery = " group by trunc(te." + queryColumn + ") ORDER BY trunc(te." + queryColumn + ")";
		query = query + tempQuery + groupQuery;
		System.err.println("get - " + query);
